...
data3 <- data[data$Date %in% c("1/2/2007","2/2/2007") ,]

dateTime <- strptime(paste(data3$Date, data3$Time, sep=" "), "%d/%m/%Y %H:%M:%S") 
globalActivePower <- as.numeric(data3$Global_active_power)
subMetering1 <- as.numeric(data3$Sub_metering_1)
subMetering2 <- as.numeric(data3$Sub_metering_2)
subMetering3 <- as.numeric(data3$Sub_metering_3)
